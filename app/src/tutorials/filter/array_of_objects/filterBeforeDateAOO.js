// filterBeforeDateAOO.js
// returns only the items from an array of objects that have a date before the given cutoff date
...
function filterBeforeDateAOO(whichArray, cutoffDate)
{
    ...
    for (let z = 0; z < whichArray.length; z++)
    {
        ...
        // if the item's date is before the cutoff, add it to the result array
        if (itemDate < cutoff)
        {
    ...
    // return the new array containing only items before the cutoff date
    return result;
}
...
// test the function
let filteredPeople = filterBeforeDateAOO(people, "2024-01-01 00:00:00");
